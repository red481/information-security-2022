...
def sdes_encrypt_ecb(text: bitarray, key: bitarray):
    text_list = []
    result_sdes = bitarray()
    text_length = len(text) // 8
    n = 0

    for i in range(1, text_length + 1):
        text_list.append(bitarray(text[n:(i*8)]))
        n = i*8

    for p_text in text_list:
        result_sdes += sdes(p_text, key, MODE_ENCRYPT)
...
def sdes_decrypt_ecb(ciphertext: bitarray, key: bitarray):
    text_list = []
    result_sdes = bitarray()
    text_length = len(ciphertext) // 8
    n = 0

    for i in range(1, text_length + 1):
        text_list.append(bitarray(ciphertext[n:(i * 8)]))
        n = i * 8

    for p_text in text_list:
        result_sdes += sdes(p_text, key, MODE_DECRYPT)
...
random_iv = bitarray(bin(random.getrandbits(7) + (1 << 7)).replace('0b', '')) # 1 << 8 을 7로 수정하였습니다.
