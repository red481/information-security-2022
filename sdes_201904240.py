...
def round(text: bitarray, round_key: bitarray) -> bitarray:
    ...
    expanded ^= round_key
    print(f'expanded xOR rondkey:{expanded}')
    # S0
    s0_row = expanded[0:4]
...
def sdes(text: bitarray, key: bitarray, mode) -> bitarray:
    ...
    key_set = schedule_keys(key)
    a_key = bitarray()
    for ip in IP:
        result.append(text[ip])

    if mode == 2:
        key_set[0], key_set[1] = key_set[1], key_set[0] #use a decrypt key

    first_switched = bitarray()
    second_switched = bitarray()

    first_switched = result[0:4]
    second_switched = result[4:]

    for phase in range(0, 2):
        a_key = key_set[phase]
        round_result = round(second_switched, a_key)
        round_result ^= first_switched
        print(f'round_result:{round_result}')
        if phase == 0:
            first_switched, second_switched = result[4:], round_result
        else:
            concat_result = round_result + second_switched
            result = bitarray()
            for ip_1 in IP_1:
                result.append(concat_result[ip_1])
